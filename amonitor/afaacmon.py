...
class gpucorrVisHdlr(logHdlr):
	...
	def __init__ (self, desc, sbnum):
		print '--> Creating GPU raw visibility handler.';
		logHdlr.__init__(self,desc);
		self._sbnum = sbnum;
		self._fobs = sbnum * 195312.5; # Hz.
	...
	def readRec (self): # Can include number of records to read here TODO
		...
		self._vis = numpy.reshape (numpy.asarray (struct.unpack ("ff"*Nbline*Nchan*Npol, rec[512:])),[Nbline, Nchan, Npol, 2]);
		...
		self._acm[:,:]= 0;
		for ind in range (0, Npol):
			self._acm [ind,self._sel] = \
					self._vis_int[:,ind,0] + 1j*self._vis_int[:,ind,1];

		# Make ACM hermitian symmetric
		self._acm_resh = \
			numpy.reshape (self._acm, [Npol, Nelem, Nelem]);
		for ind in range (0, Npol):
			self._acm_resh [ind,:,:] = \
		self._acm_resh[ind,:,:] + self._acm_resh[ind,:,:].conj().transpose();
	
		self._knownkeys['ACM']['VAL']  = self._acm_resh;
		self._knownkeys['TOBS']['VAL'] = self._tobs;
		self._knownkeys['FOBS']['VAL'] = self._fobs;
...
#class ftImager:
#	'Class to carry out Fourier imaging from a single subband of visibilities, all pols, as generated by the GPU correlator.'
#	# Public variables, shared between all class instances;
#	C = 2.9979245e8; # m/s
#
...
#		self._mode = mode;
#		self._sbnum = sbnum;
#		self._meta = ctypes.create_string_buffer(512); # 512Byte file header.
...
class subpltType:
	...
	def __init__ (self, plttype, xlab, ylab, tit, xdat, ydat=[], ylegend=[], sym='o-',grid=True):
		self._plttype = plttype;
		if (self._plttype == 'PLT'):
			self._xlab = xlab;
			self._ylab = ylab;
			self._title= tit;
			self._xdat = xdat;
		...
		elif (self._plttype == 'IMG'):
			self._xlab = xlab;
			self._ylab = ylab;
			self._title= tit;
			self._xdat = xdat;

class subpltImgType(subpltType):
	'Class representing a plot device on which images are shown'

	def __init__ (self, im, location, plttype, xlab, ylab, tit, fprefix='./',wrpng=0, pltmoon=0, ylegend=[], sym='o-',grid=True):
		subpltType.__init__(self, plttype, xlab, ylab, tit, [], [], [], sym, grid);
	# def __init__ (self, im, location, fprefix='./',  wrpng=1, pltmoon=0):
		...
		if matplotFound ==0 & ephemFound == 0:
			...
			self._wrpng = 1;
		print '--> Creating image display unit.';

	def setFigure (self, fig):
		self._fig = fig; # NOTE: Implies that further overwrites are allowed!	
	...
	def showImg (self):
		if self._pltmoon == 1:
			...
			if (self._imgplt in locals()):	
				self._imgplt.set_data (abs(self._im[0]._skymap[:,:]));
			else:
				self._imgplt = self._fig.imshow (abs(self._im[0]._skymap[:,:]), extent = [self._im[0]._l[0], self._im[0]._l[-1], self._im[0]._m[0], self._im[0]._m[-1]]);
				plt.colorbar();
				plt.show();
...
if __name__ == '__main__':
	...
	# Handle 
	if opts.gpuvissrc != 0:
		gpuvishdl = gpucorrVisHdlr (opts.gpuvissrc, 300);
		...
		im = [];
		for pol in pols:
			im.append (ftImager (512, 'fft', 60000000, 'sb0', pol));

		gpuvisimg = subpltImgType (im,'lofar','IMG','l','m','XX uncalib. image');

		subplt.append (gpuvisimg);

		"""
		# Subplot showing uncalibrated ACM ampl. 
		gpuvisampplt = subpltType ('PLT', 'Secs', 'Power', 
				'Subband ACM amplitude', 
				gpuvishdl._knownkeys['TOBS']['VAL'], 
				[gpuvishdl._knownkeys['ACM']['VAL']]); 
				
		# Subplot showing uncalibrated ACM phase. 
		gpuvisphplt = subpltType ('PLT', 'Secs', 'Deg', 
				'Subband ACM phase', 
				gpuvishdl._knownkeys['TOBS']['VAL'], 
				[gpuvishdl._knownkeys['ACM']['VAL']]); 
	...
	if opts.gpulogsrc != 0:
		...
		gpusubplt = subpltType ('PLT', 'Secs', 'Secs', 
		...
		gpuflsubplt = subpltType ('PLT', 'Secs', 'Flag %', 
	...
	if opts.pipelogsrc != 0:
		...
		pipesubplt = subpltType ('PLT', 'Sec', 'Arbit', 'ACM Frobenius norm',
	...
	if len(logsrc) > 0:
		...
		pltwin.setupPlotWin();
		# NOTE: Need to pass the figure handle, only for image displays
		if opts.gpuvissrc != 0:
			gpuvisimg.setFigure(pltwin._fig);
		...
		try:
			...
			if (opts.gpuvissrc != 0):
				...
				for pind,pol in enumerate (pols):
					im[pind].createImage (gpuvishdl._knownkeys['ACM']['VAL'], gpuvishdl._knownkeys['TOBS']['VAL'], gpuvishdl._knownkeys['FOBS']['VAL']);
			pltwin.showLogPlots();
